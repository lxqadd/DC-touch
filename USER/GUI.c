...
void LCD_ShowChar(u16 x, u16 y, u16 fc, u16 bc, u8 num, u8 size, u8 mode)
{
    ...
    if(size != 24)
        ...
        LCD_SetWindows(x, y, x + 16 - 1, y + size - 1);
    if(!mode) //非叠加方式
    {
        if(size < 24)
...
//修改记录：无
//******************************************************************
void LCD_ShowNum(u16 x, u16 y, float num, u8 len, u8 size)
{
    u8 t, temp, f = 0;
    u8 enshow = 0;
    if(num < 0)
    ...
    for(t = 0; t < len; t++)
    {
        temp = ((u32)num / mypow(10, len - t - 1)) % 10;
        if(enshow == 0 && t < (len - 1))
        {
            if(temp == 0)
            {
...
void Gui_Drawbmp16(u16 x, u16 y, const unsigned char *p) //显示40*40 QQ图片
{
    ...
    LCD_SetWindows(x, y, x + 48 - 1, y + 48 - 1); //窗口设置
    for(i = 0; i < 48 * 48; i++)
    {
        picL = *(p + i * 2);	//数据低位在前
        picH = *(p + i * 2 + 1);
